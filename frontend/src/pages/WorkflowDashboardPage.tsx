 ...
 * • Live ticket list via <GenericGrid gridName="Tickets">
...
function StatusBadge({ s }: { s: string }) {
  ...
  return (
    <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${styles[s] ?? "bg-gray-800 text-gray-300"}`}>
      {labels[s] ?? s}
    </span>
  );
}
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">🎫 Support Ticket Workflow</h2>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          <KpiTile label="Total Tickets"    value={stats.total}             icon="🎫" color="border-gray-700" />
          <KpiTile label="Open"             value={stats.open}              icon="📭" color="border-yellow-800" />
          <KpiTile label="In Progress"      value={stats.in_progress}       icon="🔧" color="border-blue-800" />
          <KpiTile label="Escalated"        value={stats.escalated}         icon="🔺" color="border-orange-800" />
          <KpiTile label="Manager Review"   value={stats.manager_escalated} icon="👔" color="border-red-800" />
          <KpiTile label="Resolved"         value={stats.resolved}          icon="✅" color="border-green-800" />
        </div>
      )}

      {/* ── Workflow Engine Info ─────────────────────────────────────────── */}
      <div className="rounded-xl border border-indigo-900/50 bg-indigo-950/20 px-5 py-3 text-sm text-indigo-300 flex items-center gap-3">
        <span className="text-lg">⚙️</span>
        <span>
          Workflow engine reads all phases, SLA timers, and signal definitions from the database.
          <span className="text-indigo-400 font-medium"> Add a new workflow type by inserting rows into workflow_definitions, workflow_phases, and workflow_sla_rules — zero code changes.</span>
        </span>
      </div>
      ...
      <div className="rounded-xl border border-gray-700 overflow-hidden">
        ...
        <div className="bg-gray-900 overflow-x-auto">
          {loading ? (
            <div className="py-12 text-center text-gray-500 text-sm">Loading tickets…</div>
          ) : filtered.length === 0 ? (
            <div className="py-12 text-center text-gray-500 text-sm">No tickets found</div>
          ) : (
                ...
                {(filtered as Record<string,unknown>[]).map((t) => {
                  ...
                  return (
                    <tr key={t.ticket_id as number}
                      ...
                      <td className="px-4 py-3 text-indigo-400 font-mono font-medium hover:underline">
                        {t.ticket_number as string}
                      </td>
                      <td className="px-4 py-3 text-gray-200">{t.customer_name as string}</td>
                      <td className="px-4 py-3 text-gray-300 max-w-[220px] truncate" title={t.subject as string}>
                        {t.subject as string}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {t.phase_label ? (
                          <span className="inline-flex items-center gap-1.5">
                            ...
                            <span className="text-xs text-gray-300">{t.phase_label as string}</span>
                          </span>
                      ...
                      <td className="px-4 py-3 text-xs">
                        {minsLeft !== null ? (
                          <span className={minsLeft < 60 ? "text-red-400 font-medium" : minsLeft < 240 ? "text-amber-400" : "text-gray-400"}>
